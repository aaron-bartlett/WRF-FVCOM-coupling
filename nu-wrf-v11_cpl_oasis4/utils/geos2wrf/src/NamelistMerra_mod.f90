...
contains
  ...
  !----------------------------------------------------------------------------
   ...
   function createNamelistMerra(filename) result (this)
      ...
      if (merraFormat_tavg1_2d_lnd_Nx .ne. HDF4_FORMAT .and. &
          merraFormat_tavg1_2d_lnd_Nx .ne. NETCDF_FORMAT .and. &
          merraFormat_tavg1_2d_lnd_Nx .ne. HDFEOS2_FORMAT) then
      ...
      do i = 1, numberOfDays
         if (trim(merraFiles_tavg1_2d_lnd_Nx(i)) .eq. 'NULL') then
            print*,'ERROR, invalid file name found!'
