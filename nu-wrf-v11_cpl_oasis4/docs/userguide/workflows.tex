...
Finally, note that these workflows can be executed, end-to-end, as defined by
regression test templates which are discussed later in this chapter.
...
\item \textbf{WRF}. This program will perform a numerical weather prediction
  simulation using the data from REAL. User needs to change the 
  ...
  the user can specify the new Goddard microphysics 
  (mp\_physics=7), and the Goddard 2017 radiation scheme
  (ra\_lw\_physics=5 for longwave, and ra\_sw\_physics=5 for 
  shortwave respectively), all without aerosol coupling. The 2017 radiation 
...
\item \textbf{LISWRFDOMAIN}. The user must provide a \texttt{ldt.config} file 
  ...
  script found in \texttt{scripts/python/utils/}).  
  ...
  Sample configuration files are provided in the  \texttt{testcases/} directory.
...
\item \textbf{LDT}. The user must further customize the \texttt{ldt.config}
  ...
  processing mode'' [see~\cite{ref:LdtUserGuide}].  The 
  following settings are recommended/supported (please refer to the LDT documentation):
  \begin{itemize}
  \item NoahMP.3.6 is the recommended land surface model;
...
\item \textbf{LIS}. The user must further customize \texttt{lis.config} for 
  ...
  are described in more detail in ~\cite{ref:LisUserGuide}.
...
(1 = USGS, 2 = MODIS, 3 = UMD); lis\_filename is an array of character
strings specifying the combined LDT/LIS files for each WRF domain; use\_lis\_noahmp
specifies that a NOAHMP LSM will be usd, otherwise (``no`` option) a regular
Noah LSM is assumed; use\_lis\_urban specifis that urban physics values generated
by the LIS framework will be updated (coupled) into the WRF surface driver.
...
A sample script (\texttt{utils/geos2wrf/scripts/merra/run\_geos2wrf\_merra2\_3hrassim.sh}) 
...
(\url{http://disc.sci.gsfc.nasa.gov}), and are 
available to the general public. MERRA-2 files are also accessible on the NASA
Discover supercomputer in\\
\texttt{/discover/nobackup/projects/gmao/merra2/merra2/scratch/}\\
but are only available to select users authorized by the GMAO.
...
\item \textbf{tavg1\_2d\_lnd\_Nx} (variable names are HDF4/netCDF or HDFEOS2):
  \begin{itemize}
  \item 'longitude' or 'XDim' or 'lon' (longitude)
  \item 'latitude' or 'YDim' or 'lat' (latitude)
  \item 'time' or 'TIME:EOSGRID' or 'TIME' (synoptic hours)
...
Note that the tavg1\_2d\_*\_Nx collections are 1-hour 
...
To download the MERRA data and run MERRA2WRF for a specified start date
and end date go to scripts/python/utils and run merra2wrf.py using the command:
\begin{quote}
  \texttt{./merra2wrf.py StartDate EndDate -o OutputDir -n NUWRFDIR}.
\end{quote}
For example:
\begin{verbatim}
...
for METGRID will be generated in a \emph{data} directory under \emph{out\_dir}.  

To use MERRA-2 reanalysis, the user may download MERRA-2 files from the
GES DISC web page and process them with MERRA2WRF got to the
utils/geos2wrf/scripts/run\_merra directory and run:
\begin{quote}
\texttt{./proc\_merra2\_ges\_disc.sh StartDate EndDate RunDir NUWRFDIR}
\end{quote}

A third alternative is to customize \texttt{namelist.merra2wrf} 
by hand to  process the selected MERRA files and \texttt{namelist.merra2\_2wrf} to process
the selected MERRA-2 files (both files are in utils/geos2wrf/namelist). The namelist files consists of a single block:
...
Running GOCART in WRF allows for aerosol coupling with the Goddard 
4ICE microphysics scheme and with the 2017 Goddard radiation 
scheme, providing simulation of the direct and indirect aerosol effects on 
...
4ICE schemes (mp\_physics=7) will be activated if the new
``gsfcgce\_gocart\_coupling'' namelist variable is set to 1 (set by 
default). Likewise, aerosol coupling with the 2017 Goddard radiation 
schemes (ra\_lw\_physics=5 and ra\_sw\_physics=5) will 
...
\texttt{scripts/python/utils/} directory.  The configuration file specifies the
...
See \url{http://data.remss.com/sst} for a description of these products.
...
The user must compile using \texttt{./build.sh sst2wrf}. A script provided in \\
\texttt{scripts/python/utils/sst2wrf.py} can be used to download SST data. Run,
./sst2wrf -h to help set the required variables and fetch the data. For example:
\begin{verbatim}
...
The program is then run by typing \texttt{./sst2wrf.x} in the same directory
...
\label{subsec:RegTest}

Regression testing is a type of software testing that verifies that software
previously developed and tested still performs correctly even after it was changed
...
or regressions may be uncovered.
...
%-------------------------------------------------------------------------------------------------------------------
\subsubsection{NU-WRF regression testing scripts}
...
A sample configuration file (sample.cfg) is included in scripts/python/regression
that can be modified and used for "personalized" testing. \\

There are three additional files that are used specifically for NU-WRF testing. Speciifically
...
\subsubsection{Workflows revisited}

In this chapter we discussed several workflows. For the purposes of regression testing
workflows are categorized as follows:

\begin{itemize}
\item   \textbf{wrf}: basic WRF-ARW runs (without LIS coupling)
\item  \textbf{wrflis}:  \textbf{wrf }but with LIS coupling (including single-colum model)
\item   \textbf{chem}:  \textbf{wrf} with chemistry (chemistry with or without LIS coupliing)
\item   \textbf{kpp}: \textbf{wrf} with chemistry using KPP
\end{itemize}

This distinction is necessary to specify test cases in the configuration file.
For a list of all supported NU-WRF configurations see the file master.cfg under
\emph{scripts/python/regression}.
...
not be specified. Upon execution you will see some messages echoed to STDOUT
but all the output will be logged to a file named $<$cfg\_file$>$.LOG.
...
Run logs will be generated in each run directory under <scratch\_dir>/results.
...
the recipient specified in the mail\_to field in USERCONFIG.
...
earlier in this chapter can be recreated with the appropriate
