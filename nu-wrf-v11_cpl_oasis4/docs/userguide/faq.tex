...
\texttt{module load comp/gcc/9.2.0}  \# For Intel C++ compiler \\
\texttt{module load comp/intel/19.1.3.304} \\
\texttt{module load mpi/impi/19.1.3.304} \\
\texttt{module load python/GEOSpyD/Ana2019.10\_py3.7} \\
\texttt{export NUWRFDIR=/discover/nobackup/projects/nu-wrf} \\
\texttt{export LIBDIR\_TAG=\$NUWRFDIR/lib/sles12/ekman/intel-intelmpi} \\
\texttt{export COMPILER\_VENDOR=intel} \\
\texttt{export MPI\_VENDOR=intelmpi} \\
\texttt{export LD\_LIBRARY\_PATH=\$LIBDIR\_TAG/jasper/lib:\$LD\_LIBRARY\_PATH} \\
\texttt{export LD\_LIBRARY\_PATH=\$LIBDIR\_TAG/gdal/lib:\$LD\_LIBRARY\_PATH} \\
\texttt{export LD\_LIBRARY\_PATH=\$LIBDIR\_TAG/eccodes/lib:\$LD\_LIBRARY\_PATH} \\
\texttt{export LD\_LIBRARY\_PATH=\$LD\_LIBRARY\_PATH:/usr/lib64} \\
\texttt{export PATH=\$LIBDIR\_TAG/netcdf4/bin:\$PATH} \\
\end{quote}
...
\texttt{module purge} \\
\texttt{unset LD\_LIBRARY\_PATH} \\
\texttt{module load /nasa/intel/impi/2019.5.281/impi/2019.5.281/intel64/modulefiles/mpi} \\
\texttt{module load gcc/6.2} \# For Intel C++ compiler \\
\texttt{module load comp-intel/2018.3.222} \# Per NAS recommendation \\
\texttt{export LD\_LIBRARY\_PATH=\$LD\_LIBRARY\_PATH:/usr/lib64} \\
\end{quote}
...
chemical transport, etc), but the settings in the \emph{testcases} directory
that provide several starting points. Each testcase contains a README file 
that summarizes the experiment. Additionally, table below shows a good first guess:
\\

\begin{tabular}{|l|l|l|l|} \hline
Category     & Selection     & Namelist           & Comments             \\ \hline
Microphysics & NU-WRF Goddard& mp\_physics=7     & Latest stable        \\
             & 4ICE          &                    & NASA option.         \\ \hline

Radiation    & NU-WRF Goddard& ra\_lw\_physics=5 & Latest stable NASA   \\
             & 2017 Radiation& ra\_sw\_physics=5 & option.              \\ \hline
...
LSM          & LIS NoahMP 3.6  & sf\_surface\_physics=55     & Spin-up LIS on  \\
