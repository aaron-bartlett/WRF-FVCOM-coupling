...
The development version supports Intel compilers.
(\texttt{ifort}, \texttt{icc}, and \texttt{icpc}) on Discover (version 19.13.304 ) and Pleiades (version 15.0.3.187). GCC (version 9.2 on Discover, 5.3 on Pleiades) is also required for library compatibility for \texttt{icpc}. The \emph{master} branch also works with GNU compilers and HPCX (OpenMPI) on Discover. 
...
compilers as NU-WRF, and it is highly recommended that \emph{static} 
library files be created and linked rather than shared object. Included with 
the NU-WRF source code are scripts (see scripts/other/baselibs.*) that facilitate the installation of
all software packages on Discover and Pleiades. The scripts can be adapted to other systems with
 modifications. The list of libraries is as follows:

\begin{itemize}
\item An MPI library. (By default, NU-WRF uses Intel MPI on Discover and
the latest SGI MPT on Pleiades. )
\item BOXMG 0.1 (Used to build with WRF\_ELEC option).
\item BUFRLIB 11.0
\item CAIRO 1.14.8
\item ECCODES 2.7.0
\item ESMF 8.0.1 compiled with MPI support.
\item FORTRANGIS 2.4
\item FREETYPE 2.7
...
\texttt{python3}, \texttt{bash}, \texttt{tcsh}, \texttt{gmake}, \texttt{sed}, 
...
Python3-based script system configured by a \emph{config} Python file, basically a text file 
...
\texttt{configure} options for the ARWPOST, RIP4, UPP, WPS, and WR components, as well as template \texttt{Makefile} names for the  GSDSU, LVT, and MET components. Not all components 
...
more fully in Appendix~\ref{sec:Porting}. 
...
Valid options are \texttt{cleanfirst}, 
  ...
  \begin{itemize} 
    ...
    (\texttt{WRF, WPS, LISF, utils}) to use alternative compilation flags set 
...
\item \texttt{Targets}. At least one target is required and multiple targets must be 
  ...
  \item The \texttt{chem} target compiles executables in the 
    \texttt{WRF/} directory -- except for LIS -- with WRF-Chem support but 
  ...
  \item The \texttt{kpp} target compiles executables in the \texttt{WRF/}
    directory -- except for LIS -- with WRF-Chem and Kinetic Pre-Processor
  ...
  \item The \texttt{lis} target compiles the LIS standalone executable in the 
  ...
  \item The \texttt{wrf} target compiles executables in the \texttt{WRF/}
    directory with LIS-coupling support and without WRF-Chem support.
    
  \item The \texttt{wrfonly} is like the \texttt{wrf} target but without LIS coupling 
    support. Use this option when not using any LISF packages and thus significantly
