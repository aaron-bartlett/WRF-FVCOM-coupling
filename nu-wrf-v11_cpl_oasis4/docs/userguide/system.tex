...
\item The \texttt{WRF/} component contains a modified copy of the core WRF 
  version 4.2.1 modeling system [see Chapter 5 of \cite{ref:ArwUserGuide}], 
  ...
  \item WRF electrification scheme: This scheme requires environment variable WRF\_ELEC=1 which can be set in the build configuration file.
  \end{itemize}
  The legacy community WRF versions of  the Goddard microphysics and radiation 
  schemes are separate options from the
  latest versions developed by GSFC. \texttt{WRF/} also includes LIS/LDT/LVT
  (in \texttt{LISF/}) with code modifications to both LIS and WRF-ARW to 
...
\item The \texttt{WPS/} component contains a modified copy of the WRF 
  Preprocessing System version 4.2.1 [see Chapter 3 
...
\item The \texttt{LISF/ldt/} component contains version 7.3 of the NASA Land 
...
\item The \texttt{LISF/lvt/} component contains version 7.3 of the NASA Land 
...
\item The \texttt{MET/} component contains version 9.1 of the Meteorological 
...
\item The \texttt{postproc/GMP/}  component contains the Goddard MicroPhysics 
simulator, an end-to-end microphysics development tool. It reads WRF output 
and conducts microphysics processes for one time step. Thus, it is a very useful 
tool for independent code development without running a whole NU-WRF simulation. 
Code can be run either in single CPU or in parallel, and users can specify
 input WRF output files and also subset domains.

\item The \texttt{postproc/GRAD/}  component contains the Goddard RADiation (GRAD) simulator,
an end-to-end radiation development tool. It reads WRF output and conducts radiative transfer 
for one time step. Like GMP, it is a very useful tool for independent code development without running 
a whole NU-WRF simulation and code can be run serially or in parallel.
...
\item The \texttt{utils/geos2wrf/} utility contains version 2 of the NASA
  ...
  MERRA, and MERRA-2.  This includes a python script for downloading and
  processing MERRA-2 files from the MERRA2 data server (only works in NCCS
  systems).
...
\item The \texttt{utils/sst2wrf/} utility contains the NASA SST2WRF 
  ...
  of UNGRIB as the SST data are in a non-GRIB binary format. This includes a
  python script for downloading data from the RSS web page.
